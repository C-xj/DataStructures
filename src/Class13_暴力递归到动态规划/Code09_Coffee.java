...
// -------------------------------优化暴力递归-------------------------------
	...
	// 洗咖啡杯的方式和原来一样，只是这个暴力版本减少了一个可变参数
	// drinks、a、b代表洗的模型
	// drinks[0..index-1]都已经干净
	// drinks[index....],洗杯子的机器什么时候可以接受新的杯子叫washLine，
	// 来到的最早时间点。
	public static int process(int[] drinks, int a, int b, int index, int washLine) {
		if (index == drinks.length - 1) { // base case : 只剩一个杯子的时候
			// 洗 or 挥发
			// 当前 咖啡喝完的时刻与咖啡杯有空的时刻的最大值+a(洗杯子的时间) 与 喝完的时间+b挥发的时间 的最小值
		...
		int next1 = process(drinks, a, b, index + 1, wash); // 在wash时刻后，index+1...变干净的最早时间
		int p1 = Math.max(wash, next1);		// 取两个时间的最大值，因为要所有杯子都变干净(选小的时间，会有不干净的杯子)

		// 想挥发
		int dry = drinks[index] + b; // 挥发完我这一杯，时间来到哪？dry
		int next2 = process(drinks, a, b, index + 1, washLine);// 在washLine时刻后，index+1...变干净的最早时间
		int p2 = Math.max(dry, next2);

		return Math.min(p1, p2);		// 返回两种选择用时最少的方式
	}

// -------------------------------原始动态规划-------------------------------
	public static int dp(int[] drinks,int a,int b){
		if (a >= b){
			return drinks[drinks.length - 1] + b;	// 最后一杯咖啡喝完+b
		}

		int N = drinks.length;
		int limit = 0; // 咖啡机什么时候可用,上面递归的washLine的最大值
		for (int i = 0 ; i < N;i++){
...
// -------------------------------优化动态规划-------------------------------
	...
	public static int minTime3(int[] arr, int n, int a, int b) {
		...
		int[] drinks = new int[n];		// n == arr.length
		for (int i = 0; i < n; i++) {
			Machine cur = heap.poll();
			cur.timePoint += cur.workTime;
			drinks[i] = cur.timePoint;
