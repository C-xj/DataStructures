...
public class Code04_Heap02 {
    ...
    public static class MyMaxHeap{
        ...
        public int pop(){
            ...
            // 将新来到堆顶的元素与子结点中最大的那个进行比较，不断交换下移，重新变成小根堆
        ...
        private void heapInsert(int[] arr , int index){
            // arr[index]
            // arr[index] 不比它的父结点小时，停止；index=0时也停止
        ...
        // 停止条件：当前结点比其子结点小；或者没有子结点
        private void heapify(int[] arr, int index, int heapSize){
            int left = index * 2 + 1;
            while (left < heapSize){
                // 两个子结点中对应值进行比较，返回值小的那个结点的索引
                //     选取右孩子的判断条件：1）有右孩子，2）右孩子的值比左孩子的小，否则为选取左孩子
                int largest = left + 1 < heapSize && arr[left + 1] < arr[left] ? left+1 : left;
                // 比较当前结点与其最小的一个子结点的大小，返回对应最小值的索引
                largest = arr[largest] < arr[index] ? largest :index;
                // 如果比较出的索引就是当前结点的索引(即，以当前结点为堆顶的堆已经是小根堆)，不再下移
                if (largest == index){
                    break;
                }
