...
public class Code03_QuickSort1 {
    ...
    public static int partition2(int[] arr ,int low ,int high){
        ...
        for (int i=low+1;i<=high;i++){
            if(arr[i]<pivot){       // 记录有多少比基准小的，mark就移动几步，mark++(即指到下一个比基准大的位置)，进行交换，让比基准小的 移到当前mark所在位置。
                mark++;
                int temp=arr[mark];
                arr[mark]=arr[i];
                arr[i]=temp;
        ...
        // 此时的arr[mark]是上面代码交换后的最后一个小于基准的数，mark位置的值与一开始基准元素交换，实现以mark对应值为基准的，左边小于区间，和右边大于区间
        arr[low]=arr[mark];
        arr[mark]=pivot;
        return mark;
    }*/
