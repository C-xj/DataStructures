...
public class Q32_合并石头的最低成本 {
    ...
    public static int process1(int L, int R, int part, int[] arr, int K, int[] presum) {
        if (L == R) { // arr[L..R]  只有一个数，要合并成一份，代价就是0。
            return part == 1 ? 0 : -1;
        }
        // L ... R  不只一个数
        if (part == 1) {   // 不只一个数,先合并成K份，最后再将K份合并为1份(累加效果)
            int next = process1(L, R, K, arr, K, presum);
            if (next == -1) {
                return -1;
            } else {
                //  l..r的累加和 = r索引之前的累加和 - l索引之前的累加和
    ...
    public static int process2(int i, int j, int part, int[] arr, int K, int[] presum, int[][][] dp) {
        ...
        if (part == 1) {
            ...
            int ans = Integer.MAX_VALUE;
            // i...mid是第1块，剩下的是part-1块，mid += K - 1 下一个必能合出来，省掉有效性检查
